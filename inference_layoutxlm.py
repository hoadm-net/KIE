"""
Inference và visualize predictions với LayoutXLM
"""
import torch
...
class LayoutXLMPredictor:
    """Predictor class cho inference với LayoutXLM"""
    ...
    def predict(
        ...
        # Inference - LayoutXLM uses 'image' parameter
        with torch.no_grad():
            # Need to rename pixel_values to image for LayoutXLM
            if "pixel_values" in encoding:
                encoding["image"] = encoding.pop("pixel_values")
    ...
    def predict_from_json(
        ...
        for entity in data["form"]:
            ...
            for idx, word_info in enumerate(entity_words):
                ...
                if idx == 0:
                    true_labels.append(f"B-{label_type}")
                else:
                    true_labels.append(f"I-{label_type}")
...
def main():
    parser = argparse.ArgumentParser(description="Inference with LayoutXLM")
    parser.add_argument(
        "--model_path",
        type=str,
        required=True,
